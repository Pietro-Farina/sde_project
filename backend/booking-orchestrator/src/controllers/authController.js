...
const completeLogin = asyncHandler(async (req, res) => {
    ...
    if (!nonce || typeof nonce !== "string") return res.sendStatus(400);

    const pending = takePendingAssertion(nonce);
    if (!pending) return res.status(400).send("No pending login (expired or already used).");
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (payload.type !== "refresh") return res.sendStatus(403);

        const allowedUser = isRefreshAllowed(payload.jti);
        if (!allowedUser || allowedUser !== payload.sub) return res.sendStatus(401);
...
const oauthAssert = asyncHandler(async (req, res) => {
    ...
    if (!provider || !sub || !nonce || !signature) return res.sendStatus(400);
...
const oauthCallback = asyncHandler(async (req, res) => {
    ...
    // Browser starts at orchestrator, orchestrator sends to adapter
    const adapterBase = process.env.OAUTH_ADAPTER_SERVICE_URL;
    res.redirect(`${adapterBase}/api/oauth/${provider}/callback`);
});
